...
import { Download, ChevronRight, Scale, Gavel, Newspaper, MessageSquareQuote, Info, FileText } from 'lucide-react';
      ...
      <main className="flex-1">
        <section className="bg-primary-dark text-white py-12">
            <div className="container mx-auto px-4">
                <h1 className="text-4xl lg:text-5xl font-bold font-poppins">Peraturan Perundang-Undangan</h1>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-right w-[100px] font-bold">Tautan</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {regulations.map(reg => (
                                            <React.Fragment key={reg.category}>
                                                <TableRow className="bg-gray-100 dark:bg-gray-800">
                                                    <TableCell colSpan={3} className="font-bold text-lg text-primary-dark dark:text-white">{reg.category}</TableCell>
                                                </TableRow>
                        ...
                        <Card>
                            ...
                            <CardContent className="p-0">
                                ...
                                {sidebarNavItems.map((item, index) => (
                                    <a key={index} href={item.href} className={`flex items-center p-4 cursor-pointer hover:bg-primary/10 transition-colors text-foreground/80 font-medium text-sm ${index !== sidebarNavItems.length - 1 ? 'border-b' : ''} ${item.label === 'REGULASI' ? 'bg-primary/10 text-primary' : ''}`}>
